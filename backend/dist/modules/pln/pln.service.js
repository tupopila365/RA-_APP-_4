...
const pln_model_1 = require("./pln.model");
...
class PLNService {
    ...
    async createApplication(dto, file) {
        try {
            ...
            let attempts = 0;
            while (attempts < 10) {
                const existing = await pln_model_1.PLNModel.findOne({ referenceId });
                if (!existing)
                    break;
            ...
            // Create application
            const application = await pln_model_1.PLNModel.create(applicationData);
            logger_1.logger.info(`PLN application created with ID: ${application._id}, Reference: ${referenceId}`);
            return application;
        }
        catch (error) {
    ...
    async getApplicationById(id) {
        try {
            const application = await pln_model_1.PLNModel.findById(id).lean();
            if (!application) {
                throw {
                    statusCode: 404,
                    code: errors_1.ERROR_CODES.NOT_FOUND,
    ...
    async listApplications(query) {
        try {
            ...
            const [applications, total] = await Promise.all([
                pln_model_1.PLNModel.find(filter)
                    .sort({ createdAt: -1 })
                    .skip(skip)
                    .limit(limit)
                    .lean(),
                pln_model_1.PLNModel.countDocuments(filter),
            ]);
            return {
    ...
    async updateStatus(id, status, adminId, comment) {
        try {
            ...
            const application = await pln_model_1.PLNModel.findById(id);
            if (!application) {
                throw {
                    statusCode: 404,
                    code: errors_1.ERROR_CODES.NOT_FOUND,
            ...
            const updated = await pln_model_1.PLNModel.findByIdAndUpdate(id, updateData, {
                new: true,
                runValidators: true,
            }).lean();
