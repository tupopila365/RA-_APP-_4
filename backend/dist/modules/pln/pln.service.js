...
exports.plnService = void 0;
const db_1 = require("../../config/db");
const pln_entity_1 = require("./pln.entity");
...
class PLNService {
    ...
    async createApplication(dto, file) {
        try {
            ...
            if (isNewStructure) {
                logger_1.logger.info('Creating PLN application (new structure):', {
                    surname: dto.surname,
                    idType: dto.idType,
                });
            }
            else {
                logger_1.logger.info('Creating PLN application (legacy structure):', {
            ...
            const repo = db_1.AppDataSource.getRepository(pln_entity_1.PLN);
            ...
            applicationData.surname_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.surname || '');
            applicationData.initials_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.initials || '');
            applicationData.surname_hash = encryption_1.FieldEncryption.hash(applicationData.surname || '');
            if (applicationData.trafficRegisterNumber) {
                applicationData.trafficRegisterNumber_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.trafficRegisterNumber);
                applicationData.trafficRegisterNumber_hash = encryption_1.FieldEncryption.hash(applicationData.trafficRegisterNumber);
            }
            if (applicationData.businessRegNumber) {
                applicationData.businessRegNumber_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.businessRegNumber);
                applicationData.businessRegNumber_hash = encryption_1.FieldEncryption.hash(applicationData.businessRegNumber);
            }
            if (applicationData.businessName) {
                applicationData.businessName_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.businessName);
            }
            if (applicationData.email) {
                applicationData.email_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.email);
                applicationData.email_hash = encryption_1.FieldEncryption.hash(applicationData.email);
            }
            if (applicationData.fullName) {
                applicationData.fullName_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.fullName);
                applicationData.fullName_hash = encryption_1.FieldEncryption.hash(applicationData.fullName);
            }
            if (applicationData.idNumber) {
                applicationData.idNumber_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.idNumber);
                applicationData.idNumber_hash = encryption_1.FieldEncryption.hash(applicationData.idNumber);
            }
            if (applicationData.phoneNumber) {
                applicationData.phoneNumber_encrypted = encryption_1.FieldEncryption.encrypt(applicationData.phoneNumber);
            }
            const application = repo.create(applicationData);
            const saved = await repo.save(application);
            logger_1.logger.info(`PLN application created with ID: ${saved.id}, Reference: ${referenceId}`);
            return saved;
        }
        catch (error) {
            logger_1.logger.error('Create PLN application error:', error);
            if (error.statusCode) {
                throw error;
            }
            throw {
    ...
    async getApplicationsByEmail(userEmail) {
        try {
            const applications = await db_1.AppDataSource.getRepository(pln_entity_1.PLN).find({
    ...
    async getApplicationById(id) {
        try {
            ...
            const application = await db_1.AppDataSource.getRepository(pln_entity_1.PLN).findOne({ where: { id: numId } });
            if (!application) {
                throw {
                    statusCode: 404,
                    code: errors_1.ERROR_CODES.NOT_FOUND,
    ...
    async listApplications(query) {
        try {
            ...
            if (query.search) {
                qb.andWhere('(p.referenceId LIKE :search OR p.fullName LIKE :search OR p.surname LIKE :search OR p.businessName LIKE :search OR p.idNumber LIKE :search OR p.trafficRegisterNumber LIKE :search OR p.businessRegNumber LIKE :search OR p.phoneNumber LIKE :search OR p.email LIKE :search)', { search: `%${query.search}%` });
            }
            if (query.startDate) {
     ...
     */
    async orderPlates(id, adminId) {
        try {
            logger_1.logger.info(`Ordering plates for application: ${id}`);
            const numId = parseId(id);
            const repo = db_1.AppDataSource.getRepository(pln_entity_1.PLN);
            const application = await repo.findOne({ where: { id: numId } });
            if (!application) {
     ...
     */
    async markReadyForCollection(id, adminId) {
        try {
            logger_1.logger.info(`Marking application ready for collection: ${id}`);
            const numId = parseId(id);
            const repo = db_1.AppDataSource.getRepository(pln_entity_1.PLN);
            const application = await repo.findOne({ where: { id: numId } });
            if (!application) {
    ...
    async getDashboardStats() {
        try {
            ...
            statusRows.forEach((item) => {
                byStatus[item.status] = parseInt(item.count, 10) || 0;
            });
            ...
            const monthlyRows = await repo
                ...
                .getRawMany();
            const monthlyStatsFormatted = monthlyRows.map((item) => ({
...
exports.plnService = new PLNService();
//# sourceMappingURL=pln.service.js.map