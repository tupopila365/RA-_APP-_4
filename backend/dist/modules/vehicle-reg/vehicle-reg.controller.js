...
exports.vehicleRegController = exports.VehicleRegController = void 0;
const vehicle_reg_service_1 = require("./vehicle-reg.service");
...
class VehicleRegController {
    ...
    async createApplication(req, res, next) {
        try {
            ...
            logger_1.logger.info('Received vehicle registration application request:', {
                body: req.body,
                hasFile: !!req.file,
                hasUser: !!userEmail,
            });
            ...
            // Create application
            const application = await vehicle_reg_service_1.vehicleRegService.createApplication(dto, req.file);
            logger_1.logger.info(`Vehicle registration application created successfully: ${application.id}`);
            res.status(201).json({
                success: true,
                data: {
                    application: {
    ...
    async getMyApplications(req, res, next) {
        try {
            ...
            const applications = await vehicle_reg_service_1.vehicleRegService.getApplicationsByEmail(userEmail);
            res.status(200).json({
                success: true,
                data: {
    ...
    async trackApplication(req, res, next) {
        try {
            ...
            const application = await vehicle_reg_service_1.vehicleRegService.getApplicationByReference(referenceId, pin);
            res.status(200).json({
                success: true,
                data: {
                    application: {
    ...
    async listApplications(req, res, next) {
        try {
            ...
            const result = await vehicle_reg_service_1.vehicleRegService.listApplications({
                page,
                limit,
                status,
                search,
    ...
    async getApplication(req, res, next) {
        try {
            const { id } = req.params;
            const application = await vehicle_reg_service_1.vehicleRegService.getApplicationById(id);
            res.status(200).json({
                success: true,
                data: {
    ...
    async updateStatus(req, res, next) {
        try {
            ...
            const application = await vehicle_reg_service_1.vehicleRegService.updateStatus(id, status, adminId, comment);
    ...
    async markPaymentReceived(req, res, next) {
        try {
            const { id } = req.params;
            const adminId = req.user?.email || req.user?.userId || 'Unknown';
            const application = await vehicle_reg_service_1.vehicleRegService.markPaymentReceived(id, adminId);
     ...
     */
    async markRegistered(req, res, next) {
        try {
            const { id } = req.params;
            const { registrationNumber } = req.body;
            const adminId = req.user?.email || req.user?.userId || 'Unknown';
            const application = await vehicle_reg_service_1.vehicleRegService.markRegistered(id, adminId, registrationNumber);
    ...
    async getDashboardStats(req, res, next) {
        try {
            const stats = await vehicle_reg_service_1.vehicleRegService.getDashboardStats();
            res.status(200).json({
                success: true,
                data: {
                    stats,
    ...
    async downloadPDF(req, res, next) {
        try {
            const { id } = req.params;
            const application = await vehicle_reg_service_1.vehicleRegService.getApplicationById(id);
            logger_1.logger.info('Downloading PDF for vehicle registration application', {
                applicationId: id,
                referenceId: application.referenceId,
            });
            ...
            try {
                ...
                if (pdf_service_1.pdfService.fillVehicleRegFormPDF) {
                    pdfBuffer = await pdf_service_1.pdfService.fillVehicleRegFormPDF(application, templatePath);
                }
                else {
                ...
                logger_1.logger.warn('Could not fill PDF, returning blank form', error);
                pdfBuffer = await fs.readFile(templatePath);
            }
            logger_1.logger.info('PDF ready', {
                bufferSize: pdfBuffer.length,
                applicationId: id,
            });
            // Set response headers
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename="Vehicle-Registration-${application.referenceId}.pdf"`);
    ...
    async updateAdminComments(req, res, next) {
        try {
            ...
            const adminId = req.user?.email || req.user?.userId || 'Unknown';
            const application = await vehicle_reg_service_1.vehicleRegService.updateAdminComments(id, comments, adminId);
    ...
    async assignToAdmin(req, res, next) {
        try {
            ...
            const application = await vehicle_reg_service_1.vehicleRegService.assignToAdmin(id, assignedTo);
    ...
    async setPriority(req, res, next) {
        try {
            ...
            const application = await vehicle_reg_service_1.vehicleRegService.setPriority(id, priority);
    ...
    async downloadForm(req, res, next) {
        try {
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', 'attachment; filename="Vehicle-Registration-Form.pdf"');
            res.setHeader('Content-Length', pdfBuffer.length.toString());
            // Send PDF
            res.send(pdfBuffer);
        }
...
exports.VehicleRegController = VehicleRegController;
exports.vehicleRegController = new VehicleRegController();
//# sourceMappingURL=vehicle-reg.controller.js.map