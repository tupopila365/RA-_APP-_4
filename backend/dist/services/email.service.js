...
class EmailService {
    ...
    initializeTransporter() {
        ...
        if (!env_1.env.SMTP_HOST || !env_1.env.SMTP_USER || !env_1.env.SMTP_PASSWORD) {
            logger_1.logger.warn('SMTP not configured - email service will not send emails');
            return;
        }
        try {
    ...
    async sendEmail(options) {
        ...
        try {
            const mailOptions = {
                from: `"${env_1.env.SMTP_FROM_NAME || 'Roads Authority Namibia'}" <${env_1.env.SMTP_FROM_EMAIL || 'noreply@roadsauthority.na'}>`,
                to: options.to,
    ...
    async sendVerificationEmail(email, fullName, token) {
        const verificationUrl = `${env_1.env.EMAIL_VERIFICATION_BASE_URL || 'roadsauthority://verify-email'}?token=${token}`;
    ...
    async sendPasswordResetEmail(email, fullName, token) {
        const resetUrl = `${env_1.env.EMAIL_VERIFICATION_BASE_URL || 'roadsauthority://reset-password'}?token=${token}`;
        const displayName = fullName || 'User';
        const html = `
      <!DOCTYPE html>
      <html>
