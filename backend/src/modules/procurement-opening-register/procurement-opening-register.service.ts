...
import type { IProcurementOpeningRegister } from './procurement-opening-register.model';
import {
...
class ProcurementOpeningRegisterService {
  async createItem(
    ...
    createdBy?: string
  ): Promise<ProcurementOpeningRegister> {
    try {
      ...
      const repo = AppDataSource.getRepository(ProcurementOpeningRegister);
      ...
      logger.info(`Procurement opening register item created with ID: ${saved.id}`);
      return saved;
    } catch (error: any) {
  ...
  async listItems(
    ...
    try {
      ...
      if (query.search) {
        qb.andWhere(
          '(p.reference LIKE :search OR p.description LIKE :search)',
          { search: `%${query.search}%` }
        );
      }
  ...
  async getItemById(id: string): Promise<ProcurementOpeningRegister> {
    try {
      const numId = parseId(id);
      const item = await AppDataSource.getRepository(ProcurementOpeningRegister).findOne({
        where: { id: numId },
      });
  ...
  async updateItem(
    ...
    try {
      ...
      if (dto.bidOpeningDate !== undefined) existing.bidOpeningDate = dto.bidOpeningDate;
      if (dto.status !== undefined) existing.status = dto.status;
      if (dto.noticeUrl !== undefined) existing.noticeUrl = dto.noticeUrl;
      if (dto.noticeFileName !== undefined) existing.noticeFileName = dto.noticeFileName;
