...
export class AuthController {
  ...
  async login(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      // #region agent log
      try { fs.appendFileSync(logPath, JSON.stringify({location:'auth.controller.ts:30',message:'AuthService.login succeeded',data:{userId:user?.id,hasTokens:!!(tokens?.accessToken&&tokens?.refreshToken)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})+'\n'); } catch(e){}
      // #endregion
