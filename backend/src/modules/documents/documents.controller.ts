...
export class DocumentsController {
   ...
   */
  async getIndexingProgress(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;

      const progress = await documentsService.getIndexingProgress(id);
      ...
      logger.error('Get indexing progress error:', error);
      next(error);
    }
  }
}
