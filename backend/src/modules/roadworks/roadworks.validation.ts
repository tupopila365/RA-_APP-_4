...
const REGIONAL_BOUNDARIES: Record<string, { center: { lat: number; lon: number }; radiusKm: number }> = {
 ...
 */
export function validateCoordinatesInNamibia(
  latitude: number,
  longitude: number
...
export function validateCoordinatesInRegion(
  ...
  if (!regionalBounds) {
    logger.warn(`No boundary data for region: ${region}`);
    return { 
      valid: true, 
 ...
 */
export function validateRoadworkData(
  dto: CreateRoadworkDTO | UpdateRoadworkDTO,
